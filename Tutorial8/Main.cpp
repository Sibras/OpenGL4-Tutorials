...
bool GL_Init()
{
    ...
    if (!GL_LoadShaderFile(uiFragmentShader, GL_FRAGMENT_SHADER, "Tutorial8Frag.glsl", 200))
        return false;
    ...
    // Load in reflection map shader
    if (!GL_LoadShaderFile(uiVertexShader, GL_VERTEX_SHADER, "ReflectionVert2.glsl", 300))
        return false;
    if (!GL_LoadShaderFile(uiTessControl, GL_TESS_CONTROL_SHADER, "PNTriangleTessControlGeom.glsl", 1300))
        return false;
    GLuint uiGeometryShader;
    if (!GL_LoadShaderFile(uiGeometryShader, GL_GEOMETRY_SHADER, "ReflectionGeom2.glsl", 400))
        return false;
    if (!GL_LoadShaders(g_uiReflectProgram, uiVertexShader, uiFragmentShader, uiGeometryShader, uiTessControl, uiTessEval))
        return false;
    ...
    glProgramUniform1i(g_uiReflectProgram, 0, g_SceneData.m_uiNumPointLights);
    glProgramUniform1i(g_uiMainProgram, 2, g_SceneData.m_uiNumSpotLights);
    glProgramUniform1i(g_uiReflectProgram, 2, g_SceneData.m_uiNumSpotLights);
...
void GL_Quit()
{
    ...
    glDeleteProgram(g_uiReflectProgram);

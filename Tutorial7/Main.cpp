...
extern bool GL_LoadShaders(GLuint & uiShader, GLuint uiVertexShader, GLuint uiFragmentShader, GLuint uiGeometryShader = -1);
...
bool GL_Init()
{
    ...
    if (!GL_LoadShaderFile(uiFragmentShader, GL_FRAGMENT_SHADER, "Tutorial7Frag.glsl", 200))
        return false;
    ...
    if (!GL_LoadShaderFile(uiVertexShader, GL_VERTEX_SHADER, "ReflectionVert.glsl", 300))
        return false;
    GLuint uiGeometryShader;
    if (!GL_LoadShaderFile(uiGeometryShader, GL_GEOMETRY_SHADER, "ReflectionGeom.glsl", 400))
        return false;
    if (!GL_LoadShaders(g_uiReflectProgram, uiVertexShader, uiFragmentShader, uiGeometryShader))
        return false;

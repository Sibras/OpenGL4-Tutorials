...
vec3 lightSpotShadow(in int iLight, in vec3 v3Position)
{
    ...
    float fAngle = random(v3Position, 500.0f) * (M_PI * 2.0f);    //***** perhaps faster 4x4 lookup instead********
    ...
    // **** TODO: add optimise to check 4 values and only continue if all 4 are not the same//
    //******TODO dudex to allow for branchning after early out samples are taken
...
float lightPointShadow(in int iLight, in vec3 v3LightDirection, in vec3 v3LightDirectionUN, in vec2 v2NearFar, in vec3 v3Position)
{
    ...
    fDepth /= (v2NearFar.y - v2NearFar.x) * fDist; //*****TODO: Should precaclulate these in host code !optimisation section!
    ...
    //******TODO dudex to allow for branching after early out samples are taken
